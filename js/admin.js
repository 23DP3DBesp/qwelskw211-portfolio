...
$('#project-form').onsubmit=async e=>{e.preventDefault();if(uploading)return;const data={};for(const name of ['id','category','status','cover','video','link','tech'])data[name]=field(name).value;data.position=Number(field('position').value);data.revision=editing?.revision;data.gallery=gallery;data.concept=field('concept').checked;for(const name of ['title','description','result'])data[name]={en:field(`${name}_en`).value,ru:field(`${name}_ru`).value};$('#save-project').disabled=true;try{await api('projects',{method:'POST',body:JSON.stringify(data)});dirty=false;await loadProjects();editing=projects.find(p=>p.id===data.id);field('id').readOnly=true;$('#editor-status').textContent='Сохранено. '+(data.status==='published'?'Работа доступна на сайте.':'Работа видна только вам.');$('#preview-project').hidden=false;$('#preview-project').href=`/work/?id=${encodeURIComponent(data.id)}&preview=1&lang=ru`;}catch(error){$('#editor-status').textContent=error.message;}finally{$('#save-project').disabled=false;}};
